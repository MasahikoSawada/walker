...
CREATE OR REPLACE FUNCTION gs_rank(
       rel regclass,
       OUT rownum INT,
       OUT percent_blocks NUMERIC(10,2),
       OUT rangeno INT,
       OUT freespace INT,
       OUT n_tuples INT,
       OUT n_dead_tuples INT,
...
SELECT
       ...
       (
              SELECT
              (percent_rank() over(order by n_dead_tuples desc) * 100)::numeric(10,2) as percent_blocks,
              *,
              round((sum(n_dead_tuples) over (order by n_dead_tuples desc range between unbounded preceding and current row))::numeric(10,2) / sum(n_dead_tuples) over () * 100, 2) as ratio
              FROM gs(rel)
